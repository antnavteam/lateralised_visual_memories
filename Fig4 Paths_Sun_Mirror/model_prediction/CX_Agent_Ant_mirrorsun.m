...
function [x,y,th,turn,err_dir,straightness, cum_CPU4_L, cum_CPU4_R, cum_PB] =CX_Agent_Ant_mirrorsun(time, mem_bias, decay_CPU4, gain, motor_noise_deg, mem_noise_deg, pontine, sun_rotation_time, sun_rotation)
...
for i=1:time %i refer to time before movement. 
    ...
    if i== sun_rotation_time(1)
        PB_thref = PB_thref + (sun_rotation/180*pi);
    end 
...
%--------------- Steering -------------
    ...
    angle_modul = turn(i)*gain ;
        % boundary
        if abs(angle_modul) > max_angle; angle_modul = max_angle; end 
        if abs(angle_modul) < min_angle; angle_modul = min_angle; end
